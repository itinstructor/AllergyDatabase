...
- The app loads the UI layout from `allergy_database.kv` which defines
...
Files of interest:
- `allergy_database.kv` — the visual layout (buttons, forms, ids).
- `database_manager.py` — the database logic (CRUD operations).
- `allergy_database.py` — ties UI and database together and contains
...
from kivy.metrics import dp, sp  # dp/sp make sizes scale across devices
from kivy.clock import Clock  # scheduling utility for delayed actions
...
class AllergyDatabaseApp(App):
    ...
    def build(self):
        ...
        # Load KV file - try local path first, then fallback
